...
public:
    ...
    static void poke8_2068(uint16_t address, uint8_t value);
    ...
    static uint16_t peek16_2068(uint16_t adddress);
    ...
    static void poke16_2068(uint16_t address, RegisterPair word);
    ...
    static void addressOnBus_2068(uint16_t address, int32_t wstates);
    ...
    static bool is2a3;
    // TS2068: SCLD memory model (8x8K memChunk[], see SCLD.h), no memory
    // contention. Kept alongside is48/is128/isPentagon/is2a3 rather than
    // folded into them since it selects an entirely different memory
    // backing store (SCLD::memChunk[]), not a variant of MemESP::ramCurrent[].
    static bool is2068;
