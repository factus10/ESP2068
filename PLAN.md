...
1. **Vendor ESPectrum into this repo.** Recommendation: a full (non-shallow) `git clone` of `EremusOne/ESPectrum` directly into this directory's history, with the original `origin` remote renamed to `upstream`. That preserves upstream history so future `git fetch upstream && git merge upstream/main` can pull fixes, without needing a GitHub fork. **Not done automatically** — creating a GitHub-side fork is a decision tied to your GitHub identity/visibility preferences, so that part is left for you to call: local-clone-with-upstream-remote is enough to start coding and costs nothing to change later if you later also want a GitHub fork for collaboration.
2. **Baseline build check.** Before changing a single line, build the unmodified `nopsram` PlatformIO env and, if hardware is on hand, flash it and confirm a stock 48K/128K boot works. This validates the toolchain (PlatformIO, ESP-IDF via `espressif32@5.4`) independent of anything this port does — if something's broken here, it's an environment problem, not a port bug, and you want to know that before slice 1 starts.
3. **License/attribution check.** Confirm `LICENSE` (GPL-3.0) and upstream attribution headers stay intact. The TS2068 build is a derivative work and inherits GPL-3.0.

Exit criteria: repo has ESPectrum's source under version control here, stock build compiles (and ideally boots on hardware), `CLAUDE.md`'s file-path claims are re-verified against the now-local checkout.
