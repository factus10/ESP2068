...
Exit criteria: repo has ESPectrum's source under version control here (done), stock build compiles (and ideally boots on hardware) (open), `CLAUDE.md`'s file-path claims are re-verified against the now-local checkout (done — one correction found: the config file is `src/ESPConfig.cpp`/`include/ESPConfig.h`, not `Config.cpp`/`Config.h`; `CLAUDE.md` updated).
